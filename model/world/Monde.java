...
public class Monde {
	 ...
	 */
	private Set gererRencontre(Agent a) {
		Set<Agent> voisins = new TreeSet<Agent>(new CoordComparator());
		for (Agent b:agents) {
			if ((Math.abs(b.getCoord().x-a.getCoord().x) < RAYON) || (Math.abs(b.getCoord().y - a.getCoord().y) < RAYON))
			{
				voisins.add(a);
			}
		}
	...
	public void cycle() {
		for (Agent a:agents) {
			Set<Agent> voisins = gererRencontre(a);
			for (Agent b:voisins) 
				a.rencontrer(b);
				a.cycle();
